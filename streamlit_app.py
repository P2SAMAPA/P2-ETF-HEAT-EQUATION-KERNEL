...
st.set_page_config(page_title="Heat Equation Kernel – Diffusion Geometry", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🔥 Heat Equation Kernel – Diffusion Geometry</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Heat diffusion on ETF correlation graph | Macro‑adapted diffusion time | Heat kernel signature</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Heat Equation")
...
st.sidebar.markdown(f"**Base diffusion time:** {config.BASE_DIFFUSION_TIME} | **Time range:** {config.DIFFUSION_RANGE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'heat_kernel_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>HKS score: {etf['heat_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized HKS"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized HKS", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🔥 Top ETFs by Heat Kernel Signature (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Heat equation kernel** on the ETF correlation graph models how information diffuses over time.
        - The **heat kernel signature (HKS)** at each node measures thermal stability / centrality.
        - Diffusion time is adapted to macro conditions: higher macro → shorter time → finer local resolution.
        - High HKS → ETF is thermally stable and central in the diffusion geometry.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the HKS scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Heat Equation Kernel | Diffusion geometry on ETF correlation graph")
