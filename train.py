...
from heat_kernel import heat_kernel_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    if len(macro_window) < len(ret_window):
        return None
    # Compute heat kernel signature for all ETFs at once
    hks = heat_kernel_score(
        ret_window,
        macro_window,
        base_time=config.BASE_DIFFUSION_TIME,
        time_range=config.DIFFUSION_RANGE,
        knn=config.KNN_NEIGHBORS
    )
    raw_scores = {ticker: float(hks[i]) for i, ticker in enumerate(ret_window.columns)}
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "heat_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "base_diffusion_time": config.BASE_DIFFUSION_TIME,
        "diffusion_range": config.DIFFUSION_RANGE,
    ...
    out_file = f"output/heat_kernel_{timestamp}.json"
